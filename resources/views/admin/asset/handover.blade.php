...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0">Handover Assets</h1>
      </div>
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="{{ url('asset') }}">Asset Management</a></li>
          <li class="breadcrumb-item active">Handover Asset</li>
        </ol>
      </div>
    </div>
  </div>
							...
							<table id="assets" class="table table-bordered table-striped table-hover">
								<thead>
									<tr>
										...
										<th> Handover Date </th>
										<th> Handovered </th>
										@if (Auth::user()->hasRole('admin') || Auth::user()->hasPermissionTo('EmployeeController:handOver'))
											<th> Actions </th>
										@endif
									</tr>
								</thead>
								<tbody>
									@foreach($handOverAssets as $asset)
										<tr>
											...
											<td class="text-nowrap">
												@if (Auth::user()->hasRole('admin') || Auth::user()->hasPermissionTo('AssetController:returnedAsset'))
													<a class="btn btn-danger btn-sm" data-toggle="modal" data-target="#confirm-returned{{ $asset->asset->id }}" data-toggle="tooltip" title="returned asset"> <i class="fas fa-pencil-alt"></i></a>
												@endif
											</td>
										</tr>
										@if (Auth::user()->hasRole('admin') || Auth::user()->hasPermissionTo('AssetController:returnedAsset'))
											<div class="modal fade" id="confirm-returned{{ $asset->asset->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
												<div class="modal-dialog">
													<div class="modal-content">
														...
														<form action="{{ route('asset.returnedasset' , $asset->asset->id )}}" method="post">
															{{ csrf_field() }}
															...
															<div class="modal-footer justify-content-between">
																...
																<input type="hidden" name="employee_id" value="{{$asset->employee->id}}" />
                                                            	<button  type="submit" class="btn btn-danger btn-ok" data-toggle="tooltip" title="Delete Job"><span class="d-xs-inline d-sm-none d-md-none d-lg-none"><i class="fas fa-trash-alt"></i></span><span class="d-none d-xs-none d-sm-inline d-md-inline d-lg-inline"> Returned</span></button>
															</div>
														</form>
													</div>
												</div>
